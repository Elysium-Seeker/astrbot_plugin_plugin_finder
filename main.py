...
@register("astrbot_plugin_plugin_finder", "插件发现者", "在对话中自行查询、校验、下载并安装 AstrBot 官方平台插件。", "1.0.0")
class PluginFinder(Star):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                for key, data in market_plugins.items():
                    ...
                    if plugin_name.lower() in key.lower() or plugin_name.lower() in data.get('display_name', '').lower():
                        if "repo" in data:
                            return {
                                "name": key,
                                "repo": data["repo"],
    ...
    async def install_plugin(self, event: AstrMessageEvent, plugin_keyword: str):
        ...
        if not plugin_info:
            yield event.plain_result(f"⚠️ 在官方插件市场中未找到 `{plugin_keyword}`，为保障安全，已终止安装。")
            return
        ...
        yield event.plain_result(f"✅ 找到官方插件：{plugin_info['display_name']} ({repo_name})\n正在执行下载...")

        # 2. 自动下载 (使用 git)
        try:
            if os.path.exists(target_dir):
                yield event.plain_result(f"发现已有同名文件夹，正在尝试更新(git pull)或重置...")
                ...
                if process.returncode != 0:
                    yield event.plain_result(f"❌ Git Clone 失败：\n{stderr.decode('utf-8', errors='ignore')}")
                    return
        except Exception as e:
            yield event.plain_result(f"❌ 源码下载环节发生错误: {str(e)}\n请确保宿主机已安装 Git。")
            return

        yield event.plain_result(f"📦 源码下载成功！正在检查依赖(requirements.txt)...")

        # 3. 自动安装依赖
        req_file = os.path.join(target_dir, "requirements.txt")
        if os.path.exists(req_file):
            yield event.plain_result(f"检测到依赖文件，正在后台执行 pip install...")
            try:
                process = await asyncio.create_subprocess_exec(
                ...
                stdout, stderr = await process.communicate()
                if process.returncode != 0:
                    yield event.plain_result(f"⚠️ 依赖安装出现错误，可能会影响插件运行：\n{stderr.decode('utf-8', errors='ignore')}")
                else:
                    yield event.plain_result(f"✅ 依赖安装完成！")
            except Exception as e:
                yield event.plain_result(f"⚠️ 依赖自动安装异常：{str(e)}")
        else:
            yield event.plain_result(f"无需额外安装依赖。")

        # 4. 热重载生效
        yield event.plain_result(f"🎉 插件 {plugin_info['display_name']} 下载及依赖安装完毕！\n"
        ...
        try:
            if hasattr(self.context, "_star_manager"):
                ...
                if success:
                    yield event.plain_result(f"✅ 热重载触发成功！新插件现已生效。")
                else:
                    yield event.plain_result(f"⚠️ 热重载触发完毕，但可能存在异常或该插件仍需全盘刷新：{err}")
            else:
                yield event.plain_result(f"⚠️ 当前版本似乎不支持后台静默重载，请发送 /plugin reload 使新插件生效。")
        except Exception as e:
            yield event.plain_result(f"⚠️ 本地安全重载API调用发生内部错误: {e}\n请手动发送 /plugin reload")
